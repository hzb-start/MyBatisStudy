...
public class TestMyBatis {
	...
	public static void queryStudentByStuname() throws IOException {
		...
		SqlSession session = sessionFactory.openSession();
		studentMapper studentmapper = session.getMapper(studentMapper.class);
		Student student = studentmapper.queryStudentByStuname("hhh");
		System.out.println(student);
		session.close();
	}
	
	public static void queryStudentOrderByStuname() throws IOException {
		Reader reader = Resources.getResourceAsReader("conf.xml");
		SqlSessionFactory sessionFactory = new SqlSessionFactoryBuilder().build(reader);
		SqlSession session = sessionFactory.openSession();
		studentMapper studentmapper = session.getMapper(studentMapper.class);
	...
	public static void addStudent() throws IOException {
		...
		SqlSessionFactory sessionFactory = new SqlSessionFactoryBuilder().build(reader);
		SqlSession session = sessionFactory.openSession();
		Student student = new Student(5, "黄朝博", 20, "hhhhhh", true);
	    studentMapper studentmapper = session.getMapper(studentMapper.class);
